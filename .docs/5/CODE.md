...
fn minimax(state: &mut GameState, depth: i32, mut alpha: i32, mut beta: i32, maximizing_player: bool) -> i32 {
...
// Function to get the best move for the computer player using Minimax with Alpha-Beta pruning
fn get_best_move(state: &mut GameState) -> (usize, usize) {
    ...
    for &(row, col) in &empty_cells {
        state.make_move(row, col);
        let eval = minimax(state, 9, alpha, beta, false);
        state.make_move(row, col); // Undo the move
